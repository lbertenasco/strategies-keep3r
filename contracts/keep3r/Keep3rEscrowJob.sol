...
contract Keep3rEscrowJob is UtilsReady, Keep3rJob, IKeep3rEscrowJob {
    ...
    function isKeep3rEscrowJob() external pure override returns (bool) {
        return true;
    }
    ...
    function getNextAction() public view override returns (IKeep3rEscrow Escrow, Actions _action) {
        ...
        if (
            Keep3rV1.liquidityAmount(address(Escrow1), address(Liquidity), address(Keep3rSugarMommy)) > 0 &&
        ...
        if (
            Keep3rV1.liquidityAmount(address(Escrow2), address(Liquidity), address(Keep3rSugarMommy)) > 0 &&
            ...
            if (_liquidityProvided > 0 && _liquidityAmount == 0) {
                ...
                if (_amount > 0) {
                    ...
                    uint256 _liquidityAmountsUnbonding =
                        Keep3rV1.liquidityAmountsUnbonding(address(OtherEscrow), address(Liquidity), address(Keep3rSugarMommy));
                    uint256 _liquidityUnbonding =
                        Keep3rV1.liquidityUnbonding(address(OtherEscrow), address(Liquidity), address(Keep3rSugarMommy));
