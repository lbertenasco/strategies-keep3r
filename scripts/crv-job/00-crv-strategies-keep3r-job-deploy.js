...
const prompt = new Confirm('Do you wish to deploy CrvStrategyKeep3rJob contract?');
    ...
    try {
      prompt.ask(async (answer) => {
        if (answer) {
          ...
          const crvStrategyKeep3rJob = await CrvStrategyKeep3rJob.deploy(escrowContracts.sugarMommy);
          
          console.timeEnd('CrvStrategyKeep3rJob deployed');
          console.log('CrvStrategyKeep3rJob address:', crvStrategyKeep3rJob.address);
          console.log('TODO: change .config.json & example.config.json crvStrategyKeep3rJob address to:', crvStrategyKeep3rJob.address);
          resolve();
        } else {
