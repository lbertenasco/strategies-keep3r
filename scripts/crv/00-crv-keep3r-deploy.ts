...
async function main() {
  await run('compile');
  const crvStrategyKeep3rContract: ContractFactory = await ethers.getContractFactory('CrvStrategyKeep3r');
  await promptAndSubmit(crvStrategyKeep3rContract);
}

function promptAndSubmit(crvStrategyKeep3rContract: ContractFactory): Promise<void|Error> {
