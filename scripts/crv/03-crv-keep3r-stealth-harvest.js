...
const prompt = new Confirm(`Do you wish to calculate crv ${process.env.STRATEGY} strategy harvest?`);
      ...
      prompt.ask(async (answer) => {
        if (answer) {
          ...
          if (!process.env.STRATEGY) {
            console.info('use STRATEGY=busd to set a strategy from config.json')
            return;
          }
          ...
          const maxGwei = 200;
          if (gasPrice.gt(gwei.mul(maxGwei))) {console.error(`gas price > ${maxGwei}gwei`); return;}
