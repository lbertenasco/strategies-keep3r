...
const prompt = new Confirm({
  message: 'Do you wish to calculate dforce keep3r strategies harvest?',
});
    ...
    try {
      prompt.run().then(async (answer) => {
        if (answer) {
          ...
          const dforceStrategyKeep3r = await ethers.getContractAt(
            'DforceStrategyKeep3r',
            config.contracts.mainnet.dforceStrategyKeep3r.address,
            deployer
          );
          ...
          for (const strategy in strategies) {
            ...
            console.log(
              `${strategy}.strategist()`,
              strategist == dforceStrategyKeep3r.address
                ? 'dforceStrategyKeep3r'
                : strategist
            );
          }
          ...
          console.log(
            'checking if workable for: dforce-usdc, dforce-usdt. please wait ...'
          );
          console.time('workable');
          for (const strategy in strategies) {
            console.log(
              `workable(${strategy})`,
              await dforceStrategyKeep3r.callStatic.workable(
                strategies[strategy].contract.address
              )
            );
          }
