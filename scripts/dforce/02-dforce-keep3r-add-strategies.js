...
const prompt = new Confirm({
  message: 'Do you wish to add dforce strategies as keep3r strategies?',
});
    ...
    try {
      prompt.run().then(async (answer) => {
        if (answer) {
          ...
          for (const strategy in strategies) {
            console.log(
              `${strategy}: ${config.contracts.mainnet[strategy].address}.setStrategist(${dforceStrategyKeep3r.address})`
            );
          }
