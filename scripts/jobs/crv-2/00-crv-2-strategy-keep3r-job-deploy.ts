...
async function main() {
  await run('compile');
  const CrvStrategyKeep3rStealthJob2: ContractFactory =
    await ethers.getContractFactory('CrvStrategyKeep3rStealthJob2');
  await promptAndSubmit(CrvStrategyKeep3rStealthJob2);
}

function promptAndSubmit(
  CrvStrategyKeep3rStealthJob2: ContractFactory
): Promise<void | Error> {
