...
describe('CrvStrategyKeep3rJob', function () {
  ...
  it('Should deploy on mainnet fork', async function () {
    ...
    const crvStrategyKeep3rJob = (
      await CrvStrategyKeep3rJob.deploy(keep3rSugarMommy.address)
    ).connect(owner);
    ...
    await crvStrategyKeep3rJob.updateRequiredHarvestAmount(
      ycrvContract.address,
      requiredHarvestAmount.mul(2)
    );
    await expect(
      crvStrategyKeep3rJob.updateRequiredHarvestAmount(ycrvContract.address, 0)
    ).to.be.revertedWith(
    ...
    await expect(
      crvStrategyKeep3rJob.removeStrategy(ycrvContract.address)
    ).to.be.revertedWith(
      'crv-strategy-keep3r::remove-strategy:strategy-not-added'
    );
    await expect(
      crvStrategyKeep3rJob.updateRequiredHarvestAmount(
        ycrvContract.address,
        requiredHarvestAmount
      )
    ).to.be.revertedWith(
    ...
    await expect(
      crvStrategyKeep3rJob.callStatic.workable(ycrvContract.address)
    ).to.be.revertedWith('crv-strategy-keep3r::workable:strategy-not-added');
    console.timeEnd('removeStrategy');

    console.time('addStrategy');
    await expect(
      crvStrategyKeep3rJob.addStrategy(ycrvContract.address, 0)
    ).to.be.revertedWith(
      'crv-strategy-keep3r::set-required-harvest:should-not-be-zero'
    );
    await crvStrategyKeep3rJob.addStrategy(
      ycrvContract.address,
      requiredHarvestAmount
    );
    await expect(
      crvStrategyKeep3rJob.addStrategy(
        ycrvContract.address,
        requiredHarvestAmount
      )
    ).to.be.revertedWith(
    ...
    console.log(
      'workable(ycrv)',
      await crvStrategyKeep3rJob.callStatic.workable(ycrvContract.address)
    );
    console.log(
      'workable(busd)',
      await crvStrategyKeep3rJob.callStatic.workable(busdContract.address)
    );
    console.log(
      'workable(sbtc)',
      await crvStrategyKeep3rJob.callStatic.workable(sbtcContract.address)
    );
    console.log(
      'workable(pool3)',
      await crvStrategyKeep3rJob.callStatic.workable(pool3Contract.address)
    );
    console.timeEnd('workable');

    console.time('work should revert on ycrv');
    await expect(
      crvStrategyKeep3rJob.work(ycrvContract.address)
    ).to.be.revertedWith(
      'keep3rSugarMommy::isKeeper:keeper-not-min-requirements'
    );
    ...
    await ycrvContract
      .connect(multisig)
      .setStrategist(crvStrategyKeep3rJob.address);
    await busdContract
      .connect(multisig)
      .setStrategist(crvStrategyKeep3rJob.address);
    await sbtcContract
    ...
    expect(await crvStrategyKeep3rJob.callStatic.workable(ycrvContract.address))
      .to.be.false;
    console.timeEnd('forceWork ycrv makes workable false');

    console.time('keeper work reverts with not-workable');
    await expect(
      crvStrategyKeep3rJob.connect(keeper).work(ycrvContract.address)
