...
describe('CrvStrategyKeep3rJob', function() {
  ...
  it('Should deploy on mainnet fork', async function() {
    ...
    await expect(crvStrategyKeep3rJob.updateRequiredHarvestAmount(ycrvContract.address, 0))
    ...
    await expect(crvStrategyKeep3rJob.removeStrategy(ycrvContract.address))
      .to.be.revertedWith('crv-strategy-keep3r::remove-strategy:strategy-not-added');
    await expect(crvStrategyKeep3rJob.updateRequiredHarvestAmount(ycrvContract.address, requiredHarvestAmount))
      .to.be.revertedWith('crv-strategy-keep3r::update-required-harvest:strategy-not-added');
    await expect(crvStrategyKeep3rJob.callStatic.workable(ycrvContract.address))
      .to.be.revertedWith('crv-strategy-keep3r::workable:strategy-not-added');
    console.timeEnd('removeStrategy')

    console.time('addStrategy')
    await expect(crvStrategyKeep3rJob.addStrategy(ycrvContract.address, 0))
      .to.be.revertedWith('crv-strategy-keep3r::set-required-harvest:should-not-be-zero');
    await crvStrategyKeep3rJob.addStrategy(ycrvContract.address, requiredHarvestAmount);
    await expect(crvStrategyKeep3rJob.addStrategy(ycrvContract.address, requiredHarvestAmount))
    ...
    await expect(crvStrategyKeep3rJob.work(ycrvContract.address))
      .to.be.revertedWith('keep3rSugarMommy::isKeeper:keeper-not-min-requirements');
    ...
    await expect(crvStrategyKeep3rJob.connect(keeper).work(ycrvContract.address))
