...
describe('DforceStrategyKeep3r', function () {
  ...
  it('Should deploy on mainnet fork', async function () {
    ...
    const dforceStrategyKeep3r = (
      await DforceStrategyKeep3r.deploy(
        config.contracts.mainnet.keep3r.address,
        ZERO_ADDRESS,
        0,
        0,
    ...
    const dforceusdcContract = await ethers.getContractAt(
      'StrategyDForceUSDC',
      config.contracts.mainnet['dforce-usdc'].address,
      owner
    );
    const dforceusdtContract = await ethers.getContractAt(
      'StrategyDForceUSDC',
      config.contracts.mainnet['dforce-usdt'].address,
      owner
    );
    ...
    await dforceStrategyKeep3r.addStrategy(
      dforceusdcContract.address,
      requiredHarvestAmount
    );
    await dforceStrategyKeep3r.addStrategy(
      dforceusdtContract.address,
      requiredHarvestAmount
    );
    ...
    expect(strategies).to.be.deep.eq([
      dforceusdcContract.address,
      dforceusdtContract.address,
    ]);
    ...
    await dforceStrategyKeep3r.updateRequiredHarvestAmount(
      dforceusdcContract.address,
      requiredHarvestAmount.mul(2)
    );
    await expect(
      dforceStrategyKeep3r.updateRequiredHarvestAmount(
        dforceusdcContract.address,
        0
      )
    ).to.be.revertedWith(
    ...
    await expect(
      dforceStrategyKeep3r.removeStrategy(dforceusdcContract.address)
    ).to.be.revertedWith(
    ...
    await expect(
      dforceStrategyKeep3r.callStatic.workable(dforceusdcContract.address)
    ).to.be.revertedWith('dforce-strategy-keep3r::workable:strategy-not-added');
    console.timeEnd('removeStrategy');

    console.time('addStrategy');
    await expect(
      dforceStrategyKeep3r.addStrategy(dforceusdcContract.address, 0)
    ).to.be.revertedWith(
      'dforce-strategy-keep3r::set-required-harvest:should-not-be-zero'
    );
    await dforceStrategyKeep3r.addStrategy(
    ...
    await expect(
      dforceStrategyKeep3r.addStrategy(
        dforceusdcContract.address,
        requiredHarvestAmount
      )
    ).to.be.revertedWith(
    ...
    console.log(
      'calculateHarvest(dforceusdc)',
      (
        await dforceStrategyKeep3r.callStatic.calculateHarvest(
          dforceusdcContract.address
        )
      ).toString()
    );
    console.log(
      'calculateHarvest(dforceusdt)',
      (
        await dforceStrategyKeep3r.callStatic.calculateHarvest(
          dforceusdtContract.address
        )
    ...
    console.log(
      'workable(dforceusdc)',
      await dforceStrategyKeep3r.callStatic.workable(dforceusdcContract.address)
    );
    console.log(
      'workable(dforceusdt)',
      await dforceStrategyKeep3r.callStatic.workable(dforceusdtContract.address)
    );
    console.timeEnd('workable');

    console.time('harvest should revert on dforceusdc');
    await expect(
      dforceStrategyKeep3r.harvest(dforceusdcContract.address)
    ...
    await dforceusdcContract
      .connect(multisig)
      .setStrategist(dforceStrategyKeep3r.address);
    await dforceusdtContract
      .connect(multisig)
      .setStrategist(dforceStrategyKeep3r.address);
    ...
    if (
      ...
      await dforceStrategyKeep3r
        .connect(keeper)
        .harvest(dforceusdtContract.address);
    ...
    await expect(
      dforceStrategyKeep3r.connect(keeper).harvest(dforceusdcContract.address)
    ).to.be.revertedWith('dforce-strategy-keep3r::harvest:not-workable');
