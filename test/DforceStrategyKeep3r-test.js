...
describe('DforceStrategyKeep3r', function() {
  ...
  it('Should deploy on mainnet fork', async function() {
    ...
    await expect(dforceStrategyKeep3r.updateRequiredHarvestAmount(dforceusdcContract.address, 0))
      .to.be.revertedWith('dforce-strategy-keep3r::set-required-harvest:should-not-be-zero');
    ...
    await expect(dforceStrategyKeep3r.removeStrategy(dforceusdcContract.address))
      .to.be.revertedWith('dforce-strategy-keep3r::remove-strategy:strategy-not-added');
    await expect(dforceStrategyKeep3r.updateRequiredHarvestAmount(dforceusdcContract.address, requiredHarvestAmount))
      .to.be.revertedWith('dforce-strategy-keep3r::update-required-harvest:strategy-not-added');
    await expect(dforceStrategyKeep3r.callStatic.workable(dforceusdcContract.address))
      .to.be.revertedWith('dforce-strategy-keep3r::workable:strategy-not-added');
    console.timeEnd('removeStrategy')

    console.time('addStrategy')
    await expect(dforceStrategyKeep3r.addStrategy(dforceusdcContract.address, 0))
      .to.be.revertedWith('dforce-strategy-keep3r::set-required-harvest:should-not-be-zero');
    await dforceStrategyKeep3r.addStrategy(dforceusdcContract.address, requiredHarvestAmount);
    await expect(dforceStrategyKeep3r.addStrategy(dforceusdcContract.address, requiredHarvestAmount))
      .to.be.revertedWith('dforce-strategy-keep3r::add-strategy:strategy-already-added');
    console.timeEnd('addStrategy')
    ...
    await expect(dforceStrategyKeep3r.harvest(dforceusdcContract.address))
    ...
    await expect(dforceStrategyKeep3r.connect(keeper).harvest(dforceusdcContract.address))
