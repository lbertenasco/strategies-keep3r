...
describe('VaultKeep3r', function() {
  ...
  it.only('Should deploy on mainnet fork', async function() {
    ...
    await expect(vaultKeep3r.updateRequiredEarnAmount(ycrvVaultContract.address, 0))
    ...
    await expect(vaultKeep3r.removeVault(ycrvVaultContract.address))
      .to.be.revertedWith('vault-keep3r::remove-vault:vault-not-added');
    await expect(vaultKeep3r.updateRequiredEarnAmount(ycrvVaultContract.address, requiredEarnAmount))
      .to.be.revertedWith('vault-keep3r::update-required-earn:vault-not-added');
    await expect(vaultKeep3r.callStatic.workable(ycrvVaultContract.address))
      .to.be.revertedWith('vault-keep3r::workable:vault-not-added');
    console.timeEnd('removeVault')

    console.time('addVault')
    await expect(vaultKeep3r.addVault(ycrvVaultContract.address, 0))
      .to.be.revertedWith('vault-keep3r::set-required-earn:should-not-be-zero');
    await vaultKeep3r.addVault(ycrvVaultContract.address, requiredEarnAmount);
    await expect(vaultKeep3r.addVault(ycrvVaultContract.address, requiredEarnAmount))
    ...
    await expect(vaultKeep3r.earn(ycrvVaultContract.address))
    ...
    await expect(vaultKeep3r.connect(keeper).earn(ycrvVaultContract.address))
