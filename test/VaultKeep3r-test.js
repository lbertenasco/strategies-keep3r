...
describe('VaultKeep3r', function () {
  ...
  it('Should deploy on mainnet fork', async function () {
    ...
    for (const vault in vaults) {
      vaults[vault].contract = await ethers.getContractAt(
        'yVault',
    ...
    expect(addedVaults).to.be.deep.eq(
      Object.values(vaults).map((vault) => vault.contract.address)
    );
    ...
    await vaultKeep3r.updateRequiredEarnAmount(
      ycrvVaultContract.address,
      requiredEarnAmount.mul(2)
    );
    await expect(
      vaultKeep3r.updateRequiredEarnAmount(ycrvVaultContract.address, 0)
    ).to.be.revertedWith('vault-keep3r::set-required-earn:should-not-be-zero');
    console.timeEnd('updateRequiredEarnAmount');

    console.time('removeVault');
    await vaultKeep3r.removeVault(ycrvVaultContract.address);
    await expect(
      vaultKeep3r.removeVault(ycrvVaultContract.address)
    ).to.be.revertedWith('vault-keep3r::remove-vault:vault-not-added');
    await expect(
      vaultKeep3r.updateRequiredEarnAmount(
        ycrvVaultContract.address,
        requiredEarnAmount
      )
    ).to.be.revertedWith('vault-keep3r::update-required-earn:vault-not-added');
    await expect(
      vaultKeep3r.callStatic.workable(ycrvVaultContract.address)
    ).to.be.revertedWith('vault-keep3r::workable:vault-not-added');
    console.timeEnd('removeVault');

    console.time('addVault');
    await expect(
      vaultKeep3r.addVault(ycrvVaultContract.address, 0)
    ).to.be.revertedWith('vault-keep3r::set-required-earn:should-not-be-zero');
    await vaultKeep3r.addVault(ycrvVaultContract.address, requiredEarnAmount);
    await expect(
      vaultKeep3r.addVault(ycrvVaultContract.address, requiredEarnAmount)
    ).to.be.revertedWith('vault-keep3r::add-vault:vault-already-added');
    console.timeEnd('addVault');

    console.time('calculateEarn');
    console.log(
      'calculateEarn(ycrvVault)',
      (
        await vaultKeep3r.callStatic.calculateEarn(ycrvVaultContract.address)
      ).toString()
    );
    console.log(
      'calculateEarn(busdVault)',
      (
        await vaultKeep3r.callStatic.calculateEarn(busdVaultContract.address)
      ).toString()
    );
    ...
    console.log(
      'workable(ycrvVault)',
      await vaultKeep3r.callStatic.workable(ycrvVaultContract.address)
    );
    console.log(
      'workable(busdVault)',
      await vaultKeep3r.callStatic.workable(busdVaultContract.address)
    );
    console.timeEnd('workable');

    console.time('earn should revert on ycrvVault');
    await expect(
      vaultKeep3r.earn(ycrvVaultContract.address)
    ...
    const lastEarnAtBefore = await vaultKeep3r.callStatic.lastEarnAt(
      ycrvVaultContract.address
    );
    ...
    expect(await vaultKeep3r.callStatic.workable(ycrvVaultContract.address)).to
      .be.false;
    console.timeEnd('forceEarn ycrvVault makes workable false');

    console.time('keeper earn reverts with not-workable');
    await expect(
      vaultKeep3r.connect(keeper).earn(ycrvVaultContract.address)
    ).to.be.revertedWith('vault-keep3r::earn:not-workable');
    console.timeEnd('keeper earn reverts with not-workable');

    const lastEarnAtAfter = await vaultKeep3r.callStatic.lastEarnAt(
      ycrvVaultContract.address
    );
