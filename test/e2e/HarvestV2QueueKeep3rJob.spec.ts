...
describe('HarvestV2QueueKeep3rJob', () => {
  ...
  it('Should deploy on mainnet fork', async function () {
    ...
    const HarvestV2QueueKeep3rJob = await ethers.getContractFactory(
    ...
    await harvestV2QueueKeep3rJob.addStrategy(
      strategies[0].address,
      1,
      mainQueue,
      [100, 100, 100, 1],
    ...
    expect(lowerCaseArray(jobStrategies)).to.be.deep.eq(
      lowerCaseArray([mainStrategy])
    );

    const strategyQueue = await harvestV2QueueKeep3rJob.strategyQueueList(
      mainStrategy
    );

    expect(lowerCaseArray(strategyQueue)).to.be.deep.eq(
      lowerCaseArray(mainQueue)
    );

    let workable = await harvestV2QueueKeep3rJob.workable(
      mainStrategy,
    ...
    await expect(
      harvestV2QueueKeep3rJob.workable(mainStrategy, strategyQueue.length)
    ).to.be.revertedWith('V2QueueKeep3rJob::main-workable:on-cooldown');
  });
});
